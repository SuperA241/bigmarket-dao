...
async function checkIsRecipient(user: string, amount: number, duration: number, claimed: number) {
	let response = await simnet.callReadOnlyFn('bme000-0-governance-token', 'get-vesting-schedule', [Cl.standardPrincipal(user)], deployer);
...
async function makeClaim(user: string) {
	let response = await simnet.callPublicFn('bme000-0-governance-token', 'core-claim', [], user);
	return response;
}

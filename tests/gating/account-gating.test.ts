...
describe('gating market  creation', () => {
	...
	it('deployer can create-market if dao bootstrapped', async () => {
		...
		const lookupRootKey = contractId2Key('ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.bme023-0-market-predicting');
		console.log(
			'bme023-0-market-predicting: 0x' + lookupRootKey + ' root= 0x' + root
			//tree
		);
	...
	it('alice cannot create-market if dao bootstrapped', async () => {
		...
		//assert(merklProof.valid)
		const lookupRootKey = contractId2Key('ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.bme023-0-market-predicting');
		console.log('bme023-0-market-predicting: 0x' + lookupRootKey + ' root= 0x' + root);
	...
	it('GENERATE TESTNET MERKLE ROOTS FOR MARKET CREATION', async () => {
		...
		const allowedCreators = [
			'ST1SV7MYKRKKDG8PHSSKZ0W66DPKRPB5KV8ACN62G',
			'ST2F4ZBBV22RF2WYR424HKX5RDN6XRK19X37YEVGG',
		...
		const lookupRootKey = contractId2Key('ST1SV7MYKRKKDG8PHSSKZ0W66DPKRPB5KV8ACN62G.bme023-0-market-scalar-pyth');
		console.log(
			'ST1SV7MYKRKKDG8PHSSKZ0W66DPKRPB5KV8ACN62G, ST2F4ZBBV22RF2WYR424HKX5RDN6XRK19X37YEVGG, ST167Z6WFHMV0FZKFCRNWZ33WTB0DFBCW9M1FW3AY, ST105HCS1RTR7D61EZET8CWNEF24ENEN3V6ARBYBJ, ST3SJD6KV86N90W0MREGRTM1GWXN8Z91PF6W0BQKM, '
		);
		console.log(
			'GENERATE TESTNET MERKLE ROOTS FOR MARKET CREATION: ST1SV7MYKRKKDG8PHSSKZ0W66DPKRPB5KV8ACN62G.bme023-0-market-scalar-pyth: 0x' + lookupRootKey + ' root= 0x' + root
			//tree
		);
		//assert(merklProof.valid)
		//merklProof = generateMerkleProof(tree, deployer);
...
async function assertContractData(marketId: number, lookupRootKey: string, leaf: string, user: string, proof: any) {
	...
	assert(
		'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.bme023-0-market-predicting' === cAddress + '.' + ddv['txsender'].contractName.content,

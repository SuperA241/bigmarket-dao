...
describe('gating market  creation', () => {
	...
	it('GENERATE TESTNET MERKLE ROOTS FOR MARKET CREATION', async () => {
		...
		const allowedCreators = [
			'ST2P1A2ECDAT2053BMYKHCN3ZTRKPX2RAKKY9WJ38',
			'ST15GF59RDTKS7YY8VMKREQ7A0C1CP1NEJE23DMJA',
		...
		const lookupRootKey = contractId2Key('ST2P1A2ECDAT2053BMYKHCN3ZTRKPX2RAKKY9WJ38.bme023-0-market-scalar-pyth');
		console.log(
		...
		console.log(
			'GENERATE TESTNET MERKLE ROOTS FOR MARKET CREATION: ST2P1A2ECDAT2053BMYKHCN3ZTRKPX2RAKKY9WJ38.bme023-0-market-scalar-pyth: 0x' + lookupRootKey + ' root= 0x' + root
			//tree
		);
		//assert(merklProof.valid)
		//merklProof = generateMerkleProof(tree, deployer);

...
describe('check actual claims vs expected for some scenarios', () => {
	it('Alice stake 100STX on YES, market resolves yes', async () => {
		...
		response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(0), Cl.principal(sbtcToken)], alice);
	...
	it('Alice stakes 100STX on yes, Bob 100STX on NO market resolves yes', async () => {
		...
		response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(0), Cl.principal(sbtcToken)], bob);
	...
	it('Alice stakes 100 STX on YES, Bob stakes 50 STX on YES, Tom stakes 200 STX on NO, Annie stakes 20 STX on NO, market resolves NO', async () => {
		...
		console.log('contractBalance: ' + stxBalances?.get(deployer + '.bme023-0-market-predicting'));
		expect(response.result).toEqual(Cl.error(Cl.uint(10006)));
		response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(0), Cl.principal(sbtcToken)], developer);
		expect(response.result).toEqual(Cl.ok(Cl.uint(328005000n)));
		response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(0), Cl.principal(sbtcToken)], annie);
...
it('Alice stakes 100 STX on YES, Bob stakes 50 STX on YES, Tom stakes 200 STX on NO, Annie stakes 20 STX on NO, market resolves NO', async () => {
	...
	response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(1), Cl.principal(sbtcToken)], alice);
	expect(response.result).toEqual(Cl.ok(Cl.uint(975150n)));
	response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(1), Cl.principal(sbtcToken)], bob);
	stxBalances = simnet.getAssetsMap().get('STX'); // Replace if contract's principal
	console.log('contractBalance: ' + stxBalances?.get(deployer + '.bme023-0-market-predicting'));
	expect(response.result).toEqual(Cl.ok(Cl.uint(4875750000000n)));
	response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(1), Cl.principal(sbtcToken)], developer);
	let data = await simnet.callReadOnlyFn('bme023-0-market-predicting', 'get-market-data', [Cl.uint(0)], alice);
	...
	data = await simnet.callReadOnlyFn('bme023-0-market-predicting', 'get-stake-balances', [Cl.uint(1), Cl.principal(annie)], annie);
	expect(data.result).toEqual(
		Cl.some(Cl.list([Cl.uint(19800000000000n), Cl.uint(0), Cl.uint(0), Cl.uint(0), Cl.uint(0), Cl.uint(0), Cl.uint(0), Cl.uint(0), Cl.uint(0), Cl.uint(0)]))
	);
	stxBalances = simnet.getAssetsMap().get('STX'); // Replace if contract's principal
	console.log('contractBalance 32: ' + stxBalances?.get(deployer + '.bme023-0-market-predicting'));

	expect(response.result).toEqual(Cl.ok(Cl.uint(19503000000000n)));
	response = await simnet.callPublicFn('bme023-0-market-predicting', 'claim-winnings', [Cl.uint(1), Cl.principal(sbtcToken)], annie);
	expect(response.result).toEqual(Cl.ok(Cl.uint(19503000000000n)));
	stxBalances = simnet.getAssetsMap().get('STX'); // Replace if contract's principal
	console.log('contractBalance 32: ' + stxBalances?.get(deployer + '.bme023-0-market-predicting'));
});

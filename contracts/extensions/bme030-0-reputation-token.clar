...
(define-public (burn (owner principal) (token-id uint) (amount uint))
  (begin
    ...
    (let ((current (default-to u0 (map-get? balances { token-id: token-id, owner: owner }))))
      ...
      (print { type: "sft_burn", token-id: token-id, amount: amount, sender: owner })
      (ok true)
    )
  )
)
  ...
  (let (
    ...
    (let (
      ...
      (let ((share (/ (* rep (var-get reward-per-epoch)) total)))
        ...
        (print { type: "big_claim", user: user, epoch: epoch, amount: share, reward-per-epoch: (var-get reward-per-epoch) })
        (ok share)
      )
    )
  )

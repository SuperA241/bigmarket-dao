...
(define-private (decode-pnau-price-update (pnau-bytes (buff 8192)) (wormhole-core-address <wormhole-core-trait>))
  (let ((cursor-pnau-header (try! (parse-pnau-header pnau-bytes)))
        (cursor-pnau-vaa-size (try! (contract-call? .hk-cursor-v2 read-uint-16 (get next cursor-pnau-header))))
        (cursor-pnau-vaa (try! (contract-call? .hk-cursor-v2 read-buff-8192-max (get next cursor-pnau-vaa-size) (some (get value cursor-pnau-vaa-size)))))
...
(define-private (parse-merkle-root-data-from-vaa-payload (payload-vaa-bytes (buff 8192)))
  (let ((cursor-payload-type (unwrap! (contract-call? .hk-cursor-v2 read-buff-4 { bytes: payload-vaa-bytes, pos: u0 }) 
          ERR_INVALID_AUWV))
        (cursor-wh-update-type (unwrap! (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-payload-type)) 
          ERR_INVALID_AUWV))
        (cursor-merkle-root-slot (unwrap! (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-wh-update-type)) 
          ERR_INVALID_AUWV))
        (cursor-merkle-root-ring-size (unwrap! (contract-call? .hk-cursor-v2 read-uint-32 (get next cursor-merkle-root-slot)) 
          ERR_INVALID_AUWV))
...
(define-private (parse-pnau-header (pf-bytes (buff 8192)))
  (let ((cursor-magic (unwrap! (contract-call? .hk-cursor-v2 read-buff-4 { bytes: pf-bytes, pos: u0 }) 
          ERR_MAGIC_BYTES))
        (cursor-version-maj (unwrap! (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-magic)) 
          ERR_VERSION_MAJ))
        (cursor-version-min (unwrap! (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-version-maj)) 
          ERR_VERSION_MIN))
        (cursor-header-trailing-size (unwrap! (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-version-min)) 
          ERR_HEADER_TRAILING_SIZE))
        (cursor-proof-type (unwrap! (contract-call? .hk-cursor-v2 read-uint-8 {
            bytes: pf-bytes,
...
(define-private (parse-and-verify-prices-updates (bytes (buff 8192)) (merkle-root-hash (buff 20)))
  (let ((cursor-num-updates (try! (contract-call? .hk-cursor-v2 read-uint-8 { bytes: bytes, pos: u0 })))
  ...
  (if (is-eq (len (get result acc)) (get limit acc))
    ...
    (if (is-eq (get index (get cursor acc)) (get next-update-index (get cursor acc)))
      ...
      (let ((cursor-update (contract-call? .hk-cursor-v2 new (get bytes acc) (some (get index (get cursor acc)))))
            (cursor-message-size (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-16 (get next cursor-update))))
            (cursor-message-type (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-8 (get next cursor-message-size))))
            (cursor-price-identifier (unwrap-panic (contract-call? .hk-cursor-v2 read-buff-32 (get next cursor-message-type))))
            (cursor-price (unwrap-panic (contract-call? .hk-cursor-v2 read-int-64 (get next cursor-price-identifier))))
            (cursor-conf (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-price))))
            (cursor-expo (unwrap-panic (contract-call? .hk-cursor-v2 read-int-32 (get next cursor-conf))))
            (cursor-publish-time (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-expo))))
            (cursor-prev-publish-time (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-publish-time))))
            (cursor-ema-price (unwrap-panic (contract-call? .hk-cursor-v2 read-int-64 (get next cursor-prev-publish-time))))
            (cursor-ema-conf (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-64 (get next cursor-ema-price))))
            (cursor-proof (contract-call? .hk-cursor-v2 advance (get next cursor-message-size) (get value cursor-message-size)))
            (cursor-proof-size (unwrap-panic (contract-call? .hk-cursor-v2 read-uint-8 cursor-proof)))
            (proof-bytes (contract-call? .hk-cursor-v2 slice (get next cursor-proof-size) (some (* MERKLE_PROOF_HASH_SIZE (get value cursor-proof-size)))))
